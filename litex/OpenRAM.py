...
class DFFRAM(Module):
    def __init__(self, width=32, size=1*kB):
        ...
        self.maskwren = Signal(4)
        self.wren_b   = Signal()
        self.cs_b     = Signal()
        self.ena     = Signal()

        self.comb += [
            self.datain.eq(self.bus.dat_w[0:32]),
            # If(self.bus.adr[9:8+log2_int(depth_cascading)+1] == d,
            If(self.bus.adr[8:8+log2_int(depth_cascading)+1] == 1,
                self.wren_b.eq(~(self.bus.we & self.bus.stb & self.bus.cyc)),
                self.bus.dat_r[0:32].eq(self.dataout),
                self.cs_b.eq(1),
                self.ena.eq(self.bus.stb & self.bus.cyc)
            ),
            # maskwren is nibble based
            self.maskwren[0].eq(self.bus.sel[0]),
            self.maskwren[1].eq(self.bus.sel[1]),
            self.maskwren[2].eq(self.bus.sel[2]),
            self.maskwren[3].eq(self.bus.sel[3]),
        ]
        # self.specials += Instance("sram_1rw1r_32_256_8_sky130",
        # self.specials += Instance("sky130_sram_2kbyte_1rw1r_32x512_8",
        #     i_clk0      = ClockSignal("sys"),
